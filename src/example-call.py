#!venv/bin/python
import os, dotenv, autobahn.asyncio.component, asyncio, autobahn.wamp.types
...
async def joined(session, details):
    while True:
        request = [
            'This is py request first argument.',
            'This is py request second argument.'
        ]
        response = await session.call('test', *request)
        print('Sent:', request)
        if isinstance(response, autobahn.wamp.types.CallResult):
            print('Received:', list(response.results))
        else:
